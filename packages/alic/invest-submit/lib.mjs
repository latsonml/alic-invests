...
export function parseRequestBody(args) {
  if (args.name && args.email && args.amount) {
    return { name: args.name, email: args.email, phone: args.phone, amount: args.amount }
  }
...
export async function processInvestRequest({ name, email, phone, amount }, env) {
...
export async function handleInvestHttpRequest({ method, origin, body }, env) {
  ...
  if (!name || !email || !phone || !amount) {
    return {
      statusCode: 400,
      headers,
      body: JSON.stringify({ error: 'Name, email, phone, and intended allocation are required.' }),
    }
  }
  ...
  await processInvestRequest({ name, email, phone, amount }, env)
